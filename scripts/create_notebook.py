"""
Utility script used once to build the Google Colab notebook that lives in
notebooks/hospital_kmeans_training.ipynb.
"""
...
CELLS = [
    ...
    nbf.v4.new_code_cell("df.describe(include='all').transpose()"),
    nbf.v4.new_markdown_cell("## 2. Exploración visual (modo oscuro)"),
    nbf.v4.new_code_cell(
        "fig = px.histogram(\n"
        "    df,\n"
        "    x='demand_index',\n"
    ...
    nbf.v4.new_markdown_cell("## 3. Método del codo para elegir *k*"),
    nbf.v4.new_code_cell(
    ...
    nbf.v4.new_markdown_cell("## 4. Entrenamiento final de K-Means"),
    nbf.v4.new_code_cell(
    ...
    nbf.v4.new_markdown_cell("## 5. Exportar centroides como `pretrained_hospitals.json`"),
    nbf.v4.new_code_cell(
...
def main() -> None:
    ...
    nb.cells = CELLS
    output_path = Path(__file__).resolve().parents[1] / "notebooks" / "hospital_kmeans_training.ipynb"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with output_path.open("w", encoding="utf-8") as handle:
        nbf.write(nb, handle)
    print(f"Notebook materializado en {output_path}")
